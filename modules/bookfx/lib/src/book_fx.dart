...
class _BookFxState extends State<BookFx> with SingleTickerProviderStateMixin {
    ...
    return SizedBox(
      ...
      child: GestureDetector(
        ...
        onScaleUpdate: (details) {
          ...
          // Kiểm tra xem có đang zoom hay không
          if (isScaling()) {
            // Nếu đang zoom, không cho phép lật trang
            canFlipPage = false;
          } else {
            // Nếu đã scale lên và đã trở về trạng thái cũ, cho phép lật trang
            canFlipPage = canFlipPage || (scale == 1.0);
          }
          // Gọi hàm xử lý sự kiện pan từ bên trong hàm xử lý sự kiện zoom
          handlePan(details);
        },

        onScaleEnd: (details) {
          handlePanEnd(details);
        },
  ...
  void handlePan(ScaleUpdateDetails details) {
    // Xử lý sự kiện pan (vuốt)
    setState(() {
      if (isAnimation) {
        return;
      }
      ...
      if (downPos.dy > size.height / 3 && downPos.dy < size.height * 2 / 3) {
        // 横向翻页
        currentA = Point(move.dx, size.height - 1);
        _p.value = PaperPoint(Point(move.dx, size.height - 1), size);
      } else {
  ...
  void handlePanEnd(ScaleEndDetails details) {
    if (isAnimation) {
      return;
    }
